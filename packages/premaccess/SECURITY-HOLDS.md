...
| `sanitize-html` | 2 medium, both on `packages/twenty-server/package.json` | Manifest-level alerts only, same shape as `@tiptap/core` below: twenty-server's upstream-maintained `package.json` pins `2.17.5` exact, while the root `twenty-server/sanitize-html` resolution lifts the workspace to 2.17.7 and the lockfile resolves exactly that. Dependabot reads the declared pin, not the resolved version. Editing that manifest is what the fork gate forbids (Dependabot's own PR #55 was closed for it). | upstream's twenty-server pins >=2.17.7 (the root resolution goes at the same time). |
| `@tiptap/core` | 1 high + 1 medium, both on `packages/twenty-front/package.json` | Manifest-level alerts only: twenty-front's own `package.json` declares `@tiptap/core` at an exact `3.30.2` and that manifest is upstream-maintained, while the root `resolutions` pin the whole `@tiptap/*` family to `3.30.5` and the lockfile resolves exactly that (`@tiptap/core@npm:3.30.5`). Dependabot reads the declared range, not the resolved version, so the alert stays open on a tree that ships the fix. Editing the manifest to close the alert is the shape the fork gate forbids. | twenty-front's own pins reach >=3.30.5 upstream (the root resolutions go at the same time). |
