...
| `sharp` (0.33.5 tree only) | high - GHSA-f88m-g3jw-g9cj, GHSA-rgj7-g3m4-5g8c | Reached only by `favicons@7.2.0` (declares `sharp: ^0.33.1`, so capped `<0.34`) and `@mintlify/prebuild` (pins `0.33.5` exact) - website and docs build tooling that is not in the scanned prod images. Patching means forcing both parents outside their own declared ranges, which the `//resolutions` contract in `package.json` rejects by name; PR #40 did it anyway and it was reverted. The rest of the tree is on 0.35.4. | `favicons` or `@mintlify/prebuild` widens its sharp range. |
